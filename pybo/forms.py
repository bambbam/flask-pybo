...
class UserCreateForm(FlaskForm):
    user_name = StringField('아이디', validators=[DataRequired('아이디는 필수 입력 항목입니다.'), Length(min=3,max=25)])
    password = PasswordField('비밀번호', validators=[
        DataRequired("비밀번호는 필수 입력 항목입니다."),EqualTo('password2','비밀번호가 일치하지 않습니다.')])
